...
router.patch("/:taskID/checklists/:action", async (ctx) => {
  ...
  try {
    switch (action.toLowerCase()) {
      ...
      case "steps": {
        const { newStep, stepIndex, checklistIndex } = ctx.request.body || {};
        const response = await taskCtrl.updateChecklistStep(session, ip, newStep, stepIndex, checklistIndex, taskID);
...
router.delete("/:taskID/:action?", async (ctx) => {
  ...
  try {
    switch(action.toLowerCase()) {
      ...
      case "checklist-due": {
        const response = await taskCtrl.removeTaskChecklistDue(session, ip, ctx.request.body.index, taskID);
      ...
      case "checklist-step": {
        const { stepIndex, checklistIndex } = ctx.request.body || {};
        const response = await taskCtrl.removeChecklistStep(session, ip, stepIndex, checklistIndex, taskID);
